...
from typing import Any, Callable, Dict, List, Tuple, Set, Union
...
class PersistentCache(CacheBase):
    ...
    def lookup(
        ...
        """
        print(f"lookup: {len(choices)=}, {name=}")
...
def cuda_compile_command(
    ...
    if dst_file_suffix == "o":
        res = _cuda_compiler() + " " + " ".join(options) + f" -c -o {dst_file} {src_file}"
    ...
    def load(cls, source_code, dst_code_suffix) -> Tuple[DLLWrapper, str, str]:
        ...
        if key not in cls.cache:
            ...
            with lock:
                ...
                if not os.path.exists(output_path):
                    cmd = cuda_compile_command([input_path], output_path, dst_code_suffix).split(" ")
                    try:
